...
1. 先設好 `ADMIN_TOKEN`（**必填**，否則管理端點會回 403 停用；若沒設會沿用 `APP_ACCESS_TOKEN`）。
...
| 變數 | 預設值 | 說明 |
| --- | --- | --- |
| `APP_ACCESS_TOKEN` | 空 | 設定後，前端會要求輸入 token 才能解析與下載。要公開給所有人用就**留空** |
| `ADMIN_TOKEN` | 沿用 `APP_ACCESS_TOKEN` | 專門保護 `/admin/` cookies 刷新的密碼。設了它就能讓前端公開、但 /admin 仍受保護 |
